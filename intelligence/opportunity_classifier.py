...
def classify_opportunity(mispricing_signal: dict, verification: dict, historical: dict,
    ...
    # --- DEEP_VALUE: a real, large, structurally-grounded edge. Checked
    # before FLOW_SCALP/NOISE_FADE since a genuinely large, verified edge
    # is the strongest kind of story and shouldn't be downgraded to a
    # shorter-horizon label just because wallet flow or momentum also
    ...
    # PREVIOUSLY required time_to_resolution >= 2.0 days ("room to be
    # patient") for EVERY deep_value signal, including mechanically
    # verified ones - this directly contradicted the explicit "daily/
    # weekly trading only" preference this system is configured for
    # (config/market_categories.yml's max_days_to_resolution): a market
    # resolving "tomorrow," which is the literal definition of a daily
    # opportunity, will almost always have UNDER 2 days left by scan
    # time. Confirmed in production logs: a 29.0pp mechanically-certain
    # arbitrage edge on a same-week weather market fell through to
    # "unclassified" (which never reaches TRADE, regardless of edge
    # size) purely because of this timing mismatch. "Room to be patient"
    # doesn't even make logical sense for arbitrage - it's pure math,
    # correct by construction regardless of when it resolves; you need
    # execution liquidity, not patience. Kept only for news-verified
    # signals (LLM-elicited estimates confirmed by real evidence), where
    # wanting SOME runway for the thesis to matter is more reasonable -
    # but even there, cut from 2 days to 4 hours, since "daily" markets
    # need same-day actionability too.
    if is_mechanically_verified:
        min_days_required = 0.0
    else:
        min_days_required = 4 / 24  # 4 hours - news-verified signals only

    if (edge_size >= _DEEP_VALUE_MIN_EDGE and (is_mechanically_verified or is_news_verified)
            and (time_to_resolution is None or time_to_resolution >= min_days_required)):
