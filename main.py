    ...
    for signal in all_signals:
        ...
        try:
            ...
            report_summaries.append({
                ...
                # Without these, an IGNORE verdict's cause was invisible in
                # the diagnostic log - "confidence=low" for an IGNORE is
                # actually a hardcoded placeholder from the early-return
                # path (see intelligence.decision_engine._ignore_report),
                # not a real computed confidence score, which was
                # genuinely misleading to read.
                "risk_flags": report.get("risk_flags", []),
                "verification_status": report.get("verification", {}).get("status") if isinstance(report.get("verification"), dict) else None,
                "liquidity_usd": m_features.get("liquidity_usd"),
                "invalidation": report.get("invalidation_conditions", ""),
                "why_this_side": report.get("why_this_side", ""),
            })
...
def _print_report_details(run_id: int, report_summaries: list):
    ...
    for r in report_summaries:
        ...
        if r.get("verdict") in ("IGNORE", "WATCH"):
            # Both _ignore_report and _watch_report hardcode
            # confidence_tier="low"/confidence_score=0.0 as PLACEHOLDERS
            # (the real computed confidence, if any was reached before
            # the early return, is discarded) - showing that as a plain
            # number was genuinely misleading either way, not just for
            # IGNORE as originally fixed.
            confidence_str = "n/a (blocked before scoring)"
        print(f"  [{r['edge_pp']:.1f}pp | {r['signal_type']}] {r['title']!r}")
        print(f"      opportunity={opp_str} risk_verdict={verdict_emoji}{r.get('verdict', '?')} "
              f"decision={r['decision']} confidence={confidence_str} -> {status}")
        if r.get("risk_flags"):
            print(f"      risk_flags: {'; '.join(r['risk_flags'])}")
        if r.get("verdict") in ("IGNORE", "WATCH"):
            print(f"      liquidity_usd={r.get('liquidity_usd')} verification_status={r.get('verification_status')}")
            # PREVIOUSLY read invalidation_conditions, which is a FIXED
            # generic string for every single WATCH verdict ("Revisit
            # once verification/precedent picture improves.") regardless
            # of what actually happened - that's why every WATCH signal
            # in the last round of logs showed identical, useless text.
            # why_this_side carries the REAL, specific reason for WATCH
            # (which of five different code paths triggered it); for
            # IGNORE, the specific reason is in invalidation_conditions
            # instead (see decision_engine._ignore_report) - so which
            # field actually has the real reason depends on the verdict.
            real_reason = r.get("why_this_side") if r.get("verdict") == "WATCH" else r.get("invalidation")
            if real_reason:
                print(f"      reason: {real_reason[:300]}")
