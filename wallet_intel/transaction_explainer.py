...
def clear_event_cache():
    """
    Clears the per-event lookup cache. Must be called at the start of each
    scan cycle in a long-running (--loop) process - this cache has no TTL,
    so without periodic clearing, "current liquidity" and resolution-rule
    text shown to users silently goes stale the longer the worker has been
    running (it's a Railway worker that loops every SCAN_INTERVAL_SECONDS,
    typically for days/weeks at a stretch - a cache that's only ever
    written to, never invalidated, contradicts the explicit "CURRENT
    liquidity" labeling in _estimate_liquidity_context's output).
    """
    _event_cache.clear()
