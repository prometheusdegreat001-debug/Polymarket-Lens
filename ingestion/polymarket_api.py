...
def fetch_events_by_categories(categories: list, max_per_category: int, tag_map: dict = None):
    ...
    for category in categories:
        ...
        if tag_id is not None:
            try:
                ...
                # PREVIOUSLY unhandled entirely - one bad category (bad tag
                # ID, network hiccup, rate limit) would crash the WHOLE
                # market scan for every category, not just this one. Now
                # one category failing just means that category's markets
                # are missing this cycle - not a total scan failure.
                failures.append(f"{category} (tag_id={tag_id}): {e}")
        ...
        if len(category_events) < max_per_category // 4:
            try:
                ...
                # PREVIOUSLY silently swallowed with no logging - fine for
                # this fallback path to fail quietly in general, but not
                # invisibly, since a systemic issue (e.g. this endpoint
                # unreachable) should still show up somewhere.
                failures.append(f"{category} public-search fallback: {e}")
    ...
    if failures:
        print(f"WARNING: {len(failures)} category pull(s) had errors:")
        for f in failures[:10]:
            print(f"  - {f}")
    if not merged:
        print("WARNING: fetch_events_by_categories returned ZERO events across ALL categories. "
              "This means every category pull failed (see failures above) or every category "
              "genuinely has zero active markets right now.")
    return merged
