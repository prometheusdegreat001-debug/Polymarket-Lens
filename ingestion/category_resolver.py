...
def resolve_category_tag_ids(desired_categories: list) -> dict:
    ...
    if unresolved:
        if _cached_tags is None:
            # Default cap (500) is very likely insufficient - Polymarket
            # has per-team, per-player, per-country tags in addition to
            # top-level categories, plausibly thousands total. If
            # weather/esports/elections/etc. happen to sort past the
            # first 500 (whatever order Gamma's /tags returns), they'd
            # never even enter the candidate pool regardless of naming -
            # confirmed as a live, real problem (7/14 categories failed
            # to resolve in production logs). Raised substantially so
            # coverage isn't silently capped.
            _cached_tags = fetch_all_tags(max_tags=5000)
...
def _find_tag(search_terms: list):
    # Pass 1: exact match on label or slug (cleanest, preferred result).
    for tag in _cached_tags:
        ...
        for term in search_terms:
            if term == label or term == slug:
                return tag
    # Pass 2: substring fallback - catches real naming variations exact
    # match would otherwise miss entirely (e.g. Polymarket labeling a tag
    # "US Elections" or "Extreme Weather" rather than the bare word).
    # Only reached if pass 1 found nothing, so it never overrides a clean
    # exact match with a looser one.
    for tag in _cached_tags:
        label = (tag.get("label") or "").strip().lower()
        slug = (tag.get("slug") or "").strip().lower()
        for term in search_terms:
            if term and (term in label or term in slug):
                return tag
    return None
