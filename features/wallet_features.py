...
def classify_trade_strategy(entry_price, holding_days) -> str:
    """
    Lightweight, explainable "what kind of trade was this" label - the
    "readable strategy" and "type of trade strategy" the wallet reports
    need, built only from data we actually have (entry price vs. the
    crowd's implied odds at the time, and how long the position was
    held), not a guess about intent.
    """
    if entry_price is not None and entry_price <= 0.15:
        return "contrarian longshot (bet heavily against the crowd's odds)"
    if holding_days is not None and holding_days <= 2:
        return "fast/news-driven entry (bought shortly before resolution)"
    if holding_days is not None and holding_days >= 14:
        return "held-to-resolution conviction (bought early, rode out interim price moves)"
    if entry_price is not None and 0.15 < entry_price <= 0.40:
        return "mispricing bet (moderate-odds position against the crowd)"
    return "standard swing entry (no extreme price or timing signature)"


def _find_earliest_buy_timestamp(event_slug, outcome, activity: list):
    if not activity:
        return None
    matches = [
        a for a in activity
        if a.get("event_slug") == event_slug and a.get("outcome") == outcome and a.get("side") == "BUY"
    ]
    if not matches:
        return None
    return min(a.get("timestamp", float("inf")) for a in matches)


def compute_longshot_pattern(closed_positions: list, activity: list = None) -> dict:
    """
    ...
    When `activity` is given, also returns the full per-trade list
    (market, entry price, entry/exit time, strategy type) - the actual
    evidence of the PATTERN, not just its aggregate stats. This is what
    lets a wallet report/alert show every qualifying position instead of
    only the single biggest win.
    """
    ...
    trades = []
    for p in qualifying:
        entry_ts = _find_earliest_buy_timestamp(p.get("event_slug"), p.get("outcome"), activity)
        exit_ts = None
        holding_days = None
        if entry_ts and p.get("end_date"):
            try:
                exit_ts = datetime.fromisoformat(p["end_date"].replace("Z", "+00:00")).timestamp()
                holding_days = round((exit_ts - entry_ts) / 86400, 1)
                if holding_days < 0:
                    holding_days = 0.0  # live/in-play entry after listed close - see contrarian_win_finder
        ...
        trades.append({
            ...
            "entry_time_display": _format_ts(entry_ts),
            "exit_timestamp": exit_ts,
            "exit_time_display": _format_ts(exit_ts),
            "holding_days": holding_days,
            "realized_pnl": p.get("realized_pnl"),
            "total_staked_usd": p.get("total_bought"),
            "strategy_type": classify_trade_strategy(p.get("avg_price"), holding_days),
        })
    trades.sort(key=lambda t: t.get("entry_timestamp") or 0)
