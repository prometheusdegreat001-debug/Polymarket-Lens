...
from features.source_quality_features import is_primary_source
...
def run_verification(market_id: str, market_url: str, market_question: str,
    ...
    # Independent cross-check against the LLM's own self-reported counts
    # above, using an editable, transparent domain-trust list rather than
    # trusting the LLM's unverified claim about its own sources - this is
    # features.source_quality_features' whole stated purpose, but it was
    # never actually wired in here before now, so "primary_source_count"
    # was pure self-report with no independent verification at all. Uses
    # the MORE CONSERVATIVE (lower) of the two counts, so a source the LLM
    # over-counts as "primary" (e.g. a random blog) can't inflate past
    # what the independent domain-trust check actually supports.
    source_urls = evidence.get("source_urls", [])
    if source_urls:
        independent_primary_count = sum(1 for u in source_urls if is_primary_source(u))
        if independent_primary_count < primary_count:
            primary_count = independent_primary_count
