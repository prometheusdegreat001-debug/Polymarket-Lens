...
from historical_context.negotiation_progress_tracker import track_progress
...
def build_report(mispricing_signal: dict, market_features: dict, market_question: str,
    ...
    # Free-tier proxy for "is this heating up" - real time-to-resolution +
    # real price momentum (see main.py's db.get_and_update_market_price
    # call, which is what makes momentum a genuine cross-scan signal
    # rather than a permanent 0.0 placeholder). Previously computed but
    # never wired into any report - dead code with no effect on output.
    negotiation_progress = track_progress(market_features)

    decision = decide(mispricing_signal, verification, historical, wallet_evaluations, market_features, market_category)
