...
TAXONOMY_LABELS = [
    ...
    "noise_wallet", "lucky_bettor", "longshot_specialist",
]
...
def classify_wallet(features: dict, luck_flags: dict) -> str:
    ...
    # Checked BEFORE the noise_wallet/small-sample check and BEFORE the
    # generic lucky_bettor check below: a confirmed longshot specialist
    # (features.wallet_features.compute_longshot_pattern) has a real,
    # repeated, capped-stake pattern - a distinct style from both "not
    # enough data yet" (noise_wallet) and "one concentrated fluke"
    # (lucky_bettor), even though it can look superficially similar to
    # either from resolved-count or win-rate alone.
    if features.get("longshot_pattern", {}).get("is_longshot_specialist"):
        return "longshot_specialist"
