...
def print_startup_report():
    ...
    paid = paid_modules()
    free = [p for p in all_profiles().values() if not p.requires_paid_api]
    print("=" * 60)
    ...
    # Previously all_profiles() was defined but never called anywhere -
    # paid_modules() alone couldn't show what's actually covering the rest
    # of the pipeline for free, which matters just as much for genuine
    # cost transparency as knowing what costs money.
    print(f"Modules running entirely free: {len(free)}")
    for p in free:
        print(f"  - {p.module_name}")
    print("=" * 60)
