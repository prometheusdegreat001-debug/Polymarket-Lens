...
def fetch_leaderboard_pool(pool_size: int, categories: list, primary_period: str,
                            secondary_periods=None) -> list:
    """
    Merges the leaderboard across MULTIPLE categories and MULTIPLE time
    periods into one deduplicated candidate pool, instead of a single
    OVERALL/one-period pull.

    secondary_periods accepts either a single period string or a list -
    the original ask was specifically "pull MONTH and DAY separately,
    then cross-reference," so DAY is included by default alongside WEEK
    (WEEK was added on top as extra corroborating evidence, not a
    replacement for DAY).
    ...
    over WEEK/DAY. Merging both fixes real, confirmed gaps rather than
    just tuning one threshold.

    Each returned entry gets `source_periods`: the set of periods it
    appeared in, independent of category - a wallet appearing in MONTH
    AND DAY (i.e. consistently ranking, not just spiking once) is
    genuinely stronger evidence of a repeatable pattern than either one
    alone - exactly the "not a one-hit wonder" cross-reference originally
    asked for. This is exposed for scoring/reporting to use as
    corroborating evidence, not as an additional hard gate - adding more
    hard gates would only shrink the pool further.
    """
    ...
    if secondary_periods is None:
        secondary_periods = []
    elif isinstance(secondary_periods, str):
        secondary_periods = [secondary_periods]

    periods = [primary_period] + [p for p in secondary_periods if p != primary_period]
    ...
    for category in categories:
        for period in periods:
            try:
                ...
                # PREVIOUSLY silently swallowed with no logging at all - if
                # EVERY pull fails (bad param, auth, rate limit, API shape
                # change), the result was zero wallets with zero visibility
                # into why. This is now printed so a "zero wallets" run
                # shows its actual cause in the console/Railway logs
                # instead of just... nothing.
                failures.append(f"{category}/{period}: {e}")
                continue
            for entry in page:
    ...
    if failures:
        print(f"WARNING: {len(failures)}/{len(categories) * len(periods)} leaderboard pulls failed:")
        for f in failures[:10]:
            print(f"  - {f}")
    if not results:
        print("WARNING: fetch_leaderboard_pool returned ZERO wallets across ALL category/period pulls. "
              "This means every single leaderboard API call failed (see failures above) or the "
              "Polymarket leaderboard endpoint itself returned empty for every combination tried.")
    return results
...
def fetch_wallet_trade_summary(wallet_address: str, max_trades: int):
    ...
    /activity's limit maxes at 500 and values above that are SILENTLY
    CLAMPED (confirmed in the official spec), not rejected. Previously
    this passed max_trades directly as `limit` (up to 600 across the
    broaden-and-retry attempts in main.py) - the API would silently hand
    back only 500, and hit_cap's comparison against the original
    (uncapped) max_trades would then wrongly say "did NOT hit the cap"
    for a wallet that actually did, since 500 < 600. That under-detection
    meant a genuinely very-active wallet's real trade count/age could be
    silently truncated without the hard-inactivity/one-hit-wonder checks
    downstream ever knowing data was incomplete.
    """
    max_trades = min(max_trades, 500)
    params = {
...
def fetch_wallet_activity_detailed(wallet_address: str, limit: int = 500):
    ...
    limit = min(limit, 500)  # same 500 hard cap as fetch_wallet_trade_summary above
    params = {
    ...
    return [
        {
            ...
            # Real field from the API, not a manual size*price recomputation -
            # avoids any rounding/units discrepancy between the two.
            "notional_usd": _to_float(a.get("usdcSize")),
...
def fetch_wallet_closed_positions(wallet_address: str, limit: int = 500):
    """
    Resolved positions - the real win/loss source (realizedPnl per
    position).

    /closed-positions caps `limit` at 50 per the official spec, and
    (unlike /activity, which explicitly documents silently clamping
    out-of-range values) does NOT document clamping behavior - meaning
    an out-of-range limit is very likely REJECTED with a 400, not
    clamped. Previously this function requested limit=500 in a single
    call, 10x over the real cap - almost certainly causing every
    wallet's closed-positions fetch to fail outright, which explains
    wallets being rejected with dossier_fetch_failed across the board.
    Fixed with real 50-per-page pagination up to the requested total.
    """
    page_cap = 50
    positions = []
    offset = 0
    while len(positions) < limit:
        params = {
            "user": wallet_address, "limit": min(page_cap, limit - len(positions)),
            "offset": offset, "sortBy": "REALIZEDPNL", "sortDirection": "DESC",
        }
        page = get_json(f"{DATA_API_BASE}/closed-positions", params=params)
        if not page:
            break
