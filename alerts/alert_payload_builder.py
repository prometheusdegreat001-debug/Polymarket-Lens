...
from features.historical_context_features import compute_historical_context_features
...
def build_payload(report: dict, wallet_profiles: list) -> dict:
    ...
    return {
        ...
        # Additive fields from the opportunity-first decision pipeline
        # (intelligence.opportunity_classifier + risk_manager.risk_check) -
        # same "additive, doesn't restructure existing fields" pattern as
        # deep_dive_explanation above. discord_formatter.py adds these as
        # NEW fields on the embed, not replacements for any existing one.
        "opportunity_type": report.get("opportunity_type", "unclassified"),
        "opportunity_label": report.get("opportunity_label", "Unclassified"),
        "classification_reason": report.get("classification_reason", ""),
        "verdict": report.get("verdict", "IGNORE"),
...
def _build_deep_dive_explanation(mispricing: dict, report: dict, historical: dict, wallet_profiles: list) -> str:
    ...
    elif signal_type == "wallet_flow":
        thesis = (
            f"{_describe_signal_basis(mispricing)}. This is NOT a price-vs-benchmark comparison - "
            f"there is no external probability estimate here. The thesis is that wallets with a "
            f"demonstrated edge are putting real money on {direction}, which is itself information "
            f"worth weighing, independent of what any price model says."
        )
    elif signal_type == "momentum_fade":
        thesis = (
            f"This market saw {_describe_signal_basis(mispricing)}. The thesis is that this is more "
            f"likely a crowd overreaction than new information fully justifying the move - this is "
            f"the highest-variance signal type this system generates, since the move itself is "
            f"currently the only evidence behind it."
        )
    else:
        source = _describe_signal_basis(mispricing)
    ...
    # Precedent depth - how much historical data the precedent_score above
    # is actually backed by. A precedent_score built from 8 similar past
    # events is a much stronger basis than one built from 1 - this context
    # was computed (features.historical_context_features) but never
    # actually surfaced anywhere before.
    hist_features = compute_historical_context_features(historical)
    if not hist_features["has_precedent_data"]:
        risks.append("No reliable historical precedent data was available for this market - the precedent score above should be weighted lightly.")
    elif hist_features["similar_event_count"] <= 1:
        risks.append(f"Historical precedent is based on only {hist_features['similar_event_count']} similar past event - a thin sample, worth weighting lightly.")
...
def _describe_signal_basis(mispricing: dict) -> str:
    """
    Single source of truth for "what is this signal actually based on,"
    in plain English - used by _plain_explanation and
    _build_deep_dive_explanation so a new signal type only needs handling
    added HERE, not independently re-guessed in multiple places (which is
    exactly how wallet_flow/momentum_fade signals initially fell through
    to an incorrect "Kalshi or AI estimate" description below, before
    this existed).
    """
    signal_type = mispricing.get("signal_type", "")
    benchmark_source = mispricing.get("benchmark_source", "")
    if signal_type == "wallet_flow":
        n = mispricing.get("_agreeing_wallet_count", "multiple")
        return f"{n} tracked wallet(s) with a proven track record independently holding the same side"
    if signal_type == "momentum_fade":
        momentum_pp = mispricing.get("_momentum", 0.0) * 100
        return f"a real, measured {abs(momentum_pp):.1f}pp price move since the last scan (no verification behind the move yet)"
    if benchmark_source == "kalshi":
        return "Kalshi (an independent, real second market)"
    if benchmark_source == "llm_estimate":
        return "an AI-researched probability estimate"
    return "an internal pricing check"
...
def _plain_explanation(mispricing: dict, report: dict) -> str:
    ...
    if signal_type == "wallet_flow":
        return (
            f"{_describe_signal_basis(mispricing)} on the {direction} side of this market - "
            f"their own real money is already positioned this way, not a price-vs-benchmark comparison."
        )
    if signal_type == "momentum_fade":
        return (
            f"This market saw {_describe_signal_basis(mispricing)}, suggesting the move may be an "
            f"overreaction worth fading toward {direction} - not yet backed by news or wallet confirmation."
        )

    source_label = _describe_signal_basis(mispricing)
    return (
