...
from alerts.discord_formatter import _enforce_embed_char_limit, _fmt_recency  # shared safety net + formatting utils only - no other coupling, see module docstring
...
def _build_embed(wallet_address: str, username: str, w: dict, wallet_context: dict = None,
    ...
    if wallet_context:
        ...
        fields.append({
            ...
            "value": (
                f"{rec_label} ({score_str}) — {wallet_context.get('behavior_label', 'unclassified')}\n"
                f"🕐 Last active: {_fmt_recency(wallet_context.get('days_since_last_trade'))}\n"
                f"🎯 {wallet_context.get('copy_command') or 'No copy command available.'}"
            ),
            "inline": False,
        })
