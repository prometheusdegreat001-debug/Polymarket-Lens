...
def run_scan(categories: list, max_per_category: int, max_kalshi: int, include_wallet_scan: bool = False):
    ...
    for signal in all_signals:
        ...
        # Real price momentum (current vs. last time we scanned this same
        # market), persisted across scan cycles - see
        # storage.db.get_and_update_market_price's docstring. Without this,
        # historical_context.negotiation_progress_tracker.track_progress
        # would always see price_momentum=0.0 by construction (Gamma only
        # ever returns a live snapshot, never history) and permanently
        # report "price stable" regardless of what actually happened.
        prior_price = db.get_and_update_market_price(
            signal["market_id"], m_features.get("implied_probability")
        )
        current_price = m_features.get("implied_probability")
        m_features["price_momentum"] = (
            round(current_price - prior_price, 4)
            if prior_price is not None and current_price is not None else 0.0
        )
...
def _run_wallet_scan(run_id: int, events: list = None, closed_events: list = None):
    ...
    for wallet_record in qualifying:
        ...
        # PREVIOUSLY gated on `is_new` (only ever fired once per wallet,
        # ever - see wallet_alerts_sent's docstring in storage/db.py for
        # the full bug this fixes). Now fires whenever the recommendation
        # or score has meaningfully changed, or the cooldown has passed -
        # independent of is_new, same principle as the contrarian alert
        # dedup above.
        if wallet_record.get("copy_trade_recommendation") == "avoid":
            ...
            if should_alert and send_wallet_alert(wallet_record):
