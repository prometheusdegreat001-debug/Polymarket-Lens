...
def analyze_strategy_drift(activity: list, closed_positions: list, event_category_map: dict = None) -> dict:
    ...
    cusum_alarm, cusum_strength = _lightweight_cusum(recent, event_category_map, established_categories)
...
def _lightweight_cusum(recent_trades: list, event_category_map: dict, established_categories: set = None) -> tuple:
    """
    ...
    established_categories fix (same principle as adherence_rate and
    peripheral_dominance elsewhere in this file): a wallet with a real,
    established multi-pattern repertoire will NATURALLY spend a few weeks
    leaning harder into ONE of its own established patterns sometimes (a
    big esports tournament, a Fed announcement, whatever creates
    opportunity that period) - that alone produces exactly the "HHI
    trending in one direction" signal this function looks for, even
    though the wallet never left its established repertoire. Without this
    fix, that normal situational emphasis gets misread as drift. Only
    buckets built ENTIRELY from established categories are checked this
    way for a multi-pattern wallet - a trend driven by expansion into
    genuinely new categories still gets caught (that's what
    peripheral_dominance/novel_share are for, and they aren't bypassed
    here).
    """
    ...
    if established_categories:
        # If every trade across the whole recent window stays within the
        # wallet's own established repertoire, a concentration swing is
        # just normal rotation between known patterns, not drift - skip
        # the alarm regardless of how strong the swing looks.
        all_cats_in_window = {event_category_map.get(t.get("event_slug"), t.get("event_slug"))
                               for t in sorted_trades}
        if all_cats_in_window and all_cats_in_window.issubset(established_categories):
            return False, "none"
