...
def _print_report_details(run_id: int, report_summaries: list):
    ...
    for r in report_summaries:
        ...
        if r.get("verdict") == "IGNORE":
            # "confidence=low" here is a HARDCODED PLACEHOLDER from
            # decision_engine._ignore_report's early return, not a real
            # computed value - showing it as a plain number was
            # genuinely misleading (looked like the signal itself was
            # evaluated as weak, when really risk_check blocked it
            # before confidence was ever computed at all).
        ...
        if r.get("verdict") in ("IGNORE", "WATCH"):
            ...
            if r.get("invalidation"):
                print(f"      reason: {r['invalidation'][:200]}")
