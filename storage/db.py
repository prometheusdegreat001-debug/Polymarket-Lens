...
-- News ingestion (Layer 2, backend only - see ingestion/news_sources.py
-- and news_intelligence/). Deduplicated by URL: the same story frequently
-- gets pulled by multiple queries/categories in one ingestion pass, and
-- can reappear across separate ingestion cycles - INSERT OR IGNORE on the
-- unique url keeps exactly one row per real article regardless of how
...
-- One row per (article, market) relevance match, so the same article can
-- legitimately match several markets without duplicating the article
-- itself.
...
-- news_articles grows continuously during --loop operation (fresh
-- ingestion every NEWS_INGEST_EVERY_N_RUNS cycles, indefinitely) and
-- get_recent_news_articles range-queries + sorts by published_at on
-- EVERY scan cycle - without this index, that query degrades as the
-- table grows into the thousands of rows over weeks of continuous
-- operation. See prune_old_news_articles below for the other half of
-- keeping this table's growth in check. Placed AFTER the table's own
-- CREATE TABLE statement since SQL scripts execute top-to-bottom -
-- indexing a table before it exists is a syntax/ordering error.
...
def save_news_article(article: dict) -> int:
    """
    Inserts a normalized article (see ingestion.news_sources' normalize
    functions). Returns the article's DB id (existing or newly inserted).
    Deduplicated on url - see news_articles' schema comment.
    """
    import json as _json
    with get_conn() as conn:
        conn.execute(
            """INSERT OR IGNORE INTO news_articles
               (source_api, external_id, headline, description, url, source_name,
                published_at, country, category, keywords, language, ingested_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                article.get("source_api"), article.get("external_id"), article.get("headline"),
                article.get("description"), article.get("url"), article.get("source_name"),
                article.get("published_at"), _json.dumps(article.get("country") or []),
                _json.dumps(article.get("category") or []), _json.dumps(article.get("keywords") or []),
                article.get("language"), _now(),
            ),
        )
        row = conn.execute("SELECT id FROM news_articles WHERE url = ?", (article.get("url"),)).fetchone()
    return row["id"] if row else None


def get_recent_news_articles(max_age_hours: float = 48) -> list:
    import json as _json
    import time as _time
    cutoff = _time.time() - max_age_hours * 3600
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT * FROM news_articles WHERE published_at IS NULL OR published_at >= ? ORDER BY published_at DESC",
            (cutoff,),
        ).fetchall()
    articles = []
    for r in rows:
        a = dict(r)
        for field in ("country", "category", "keywords"):
            try:
                a[field] = _json.loads(a[field]) if a[field] else []
...
def save_news_market_match(article_id: int, market_id: str, overlap_score: float, direction: str):
    with get_conn() as conn:
        conn.execute(
            """INSERT INTO news_market_matches (article_id, market_id, overlap_score, direction, matched_at)
               VALUES (?, ?, ?, ?, ?)
               ON CONFLICT(article_id, market_id) DO UPDATE SET
                   overlap_score=excluded.overlap_score, direction=excluded.direction,
                   matched_at=excluded.matched_at""",
            (article_id, market_id, overlap_score, direction, _now()),
        )


def prune_old_news_articles(max_age_hours: float) -> int:
    """
    Deletes articles older than max_age_hours - anything past that window
    is never returned by get_recent_news_articles anyway (same cutoff),
    so keeping them around forever only costs storage and query time for
    zero benefit. Also cleans up their now-orphaned news_market_matches
    rows. Returns the number of articles deleted.
    """
    import time as _time
    cutoff = _time.time() - max_age_hours * 3600
    with get_conn() as conn:
        conn.execute(
            "DELETE FROM news_market_matches WHERE article_id IN "
            "(SELECT id FROM news_articles WHERE published_at IS NOT NULL AND published_at < ?)",
            (cutoff,),
        )
        cur = conn.execute(
...
def get_news_matches_for_market(market_id: str, limit: int = 15) -> list:
    """
    Retrieves STORED matches for a market (as opposed to
    news_intelligence.news_market_matcher.match_articles_to_market, which
    recomputes matches live each scan cycle). No caller yet in this pass -
    intentionally prepared for the next phase (a decision-support/
    dashboard layer reviewing past matches without recomputing them) per
    the user's own explicit "backend first, no UI yet" scoping. Not dead
    code in the sense of being superseded or abandoned - it's forward
    infrastructure for work not yet started.
    """
    with get_conn() as conn:
        rows = conn.execute(
            """SELECT na.*, nmm.overlap_score, nmm.direction, nmm.matched_at
               FROM news_market_matches nmm JOIN news_articles na ON na.id = nmm.article_id
               WHERE nmm.market_id = ? ORDER BY nmm.overlap_score DESC LIMIT ?""",
            (market_id, limit),
        ).fetchall()
