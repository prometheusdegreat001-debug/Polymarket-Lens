...
def _build_wallet_embed(wallet_profile: dict) -> dict:
    ...
    # Was only ever in the markdown full_report_text, never the actual
    # Discord embed people see day to day - added here so it's visible
    # without opening the full report separately.
    fields.append({
        "name": "🎯 Copy Command",
        "value": wallet_profile.get("copy_command", "Not available.")[:500],
        "inline": False,
    })
