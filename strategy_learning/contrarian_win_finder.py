...
def find_contrarian_big_win(closed_positions: list, activity: list, closed_events_pool: list = None) -> dict:
    ...
    holding_days = _compute_holding_duration(win, entry_info)
    rationale = _speculate_rationale(win, entry_info, holding_days)
    similar_events = (
...
def _compute_holding_duration(win: dict, entry_info: dict) -> float:
    """
    ...
    try:
        ...
        return None
    return round((end_ts - entry_ts) / 86400, 1)


def _speculate_rationale(win: dict, entry_info: dict, holding_days) -> str:
    """
