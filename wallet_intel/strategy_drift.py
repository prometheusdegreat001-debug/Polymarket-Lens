...
def analyze_strategy_drift(activity: list, closed_positions: list, event_category_map: dict = None) -> dict:
    ...
    # Peripheral dominance is judged by RECENT trading volume share outside
    # the established core category, not by comparing signed lifetime PnL
    # (which gets muddied when peripheral trades are simply losing money -
    # that's itself a red flag, not "not dominant" just because it's negative).
    ...
    reasons = _build_reasons(adherence_pct, cusum_alarm, cusum_strength, peripheral_dominance, drift_status)
...
def _build_reasons(adherence_pct, cusum_alarm, cusum_strength, peripheral_dominance, drift_status) -> list:
    ...
    if peripheral_dominance:
        reasons.append("More PnL is coming from categories OUTSIDE this wallet's established core focus than from it.")
