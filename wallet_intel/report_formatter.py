...
**Copy Command:**
{wallet_record.get("copy_command", "Not available.")}
...
def render_wallet_report(wallet_record: dict) -> str:
    ...
    recommendation_key = wallet_record.get("copy_trade_recommendation")  # "copy"/"watch"/"avoid", overrides already applied
    active_days = wallet_record.get("avg_active_days_per_week", 0.0)
    pattern_label = wallet_record.get("activity_pattern_label", "unknown")
    pattern_display = _ACTIVITY_PATTERN_DISPLAY.get(pattern_label, "Unknown")
    recommendation = _score_to_recommendation_label(score, recommendation_key)
    risk_level = _score_to_risk_level(score, recommendation_key)
    ...
    open_positions_section = _build_open_positions_section(wallet_record)
...
def _build_open_positions_section(wallet_record: dict) -> str:
    """
    Per-position detail for currently-open positions - previously the
    report only ever showed an aggregate count/dollar total
    (open_positions_count / open_exposure_usd), never the actual
    positions. Since "must have open positions" is one of the explicit
    copytrading requirements, this makes that requirement visible and
    checkable rather than implied by a bare number.
    """
    detail = wallet_record.get("open_positions_detail") or []
    if not detail:
        count = wallet_record.get("open_positions_count", 0)
        if count:
            return f"{count} open position(s) - detail unavailable for this record."
        return "No open positions - this wallet currently has no live exposure to copy into."

    lines = []
    for p in detail:
        entry = f"${p['entry_price']:.2f}" if p.get("entry_price") is not None else "n/a"
        cur = f"${p['current_price']:.2f}" if p.get("current_price") is not None else "n/a"
        pnl = p.get("unrealized_pnl_usd")
        pnl_str = f"${pnl:,.0f} unrealized" if pnl is not None else "unrealized PnL n/a"
        stake_note = " (stake ≤$100)" if p.get("stake_under_100") else ""
        lines.append(
            f"- **{p.get('market_title', 'unknown market')}** ({p.get('outcome', 'unknown')}): "
            f"entered @ {entry}, currently @ {cur} - {pnl_str}{stake_note}"
        )
    exposure = wallet_record.get("open_exposure_usd")
    header = f"{len(detail)} open position(s)" + (f", ${exposure:,.0f} total exposure" if exposure else "")
    return header + ":\n" + "\n".join(lines)


def _score_to_recommendation_label(score: int, recommendation_key: str = None) -> str:
    """
    IMPORTANT: recommendation_key, when given, is wallet_record's
    copy_trade_recommendation ("copy"/"watch"/"avoid") from
    wallet_intel/copy_trade_filter.py's evaluate_wallet() - the SAME
    verdict shown elsewhere in this report (Copy-Trading Fitness Verdict,
    Overall Copy Fitness) and used by main.py to decide whether to alert
    at all. That verdict already has the dormancy-override and
    drift-override applied on top of the raw score - overrides that a
    plain score threshold below knows nothing about. Without passing it
    in, a dormant or drift-flagged wallet with a good historical raw
    score could show "Strong Buy" here while correctly showing "No"
    further down in the SAME report - which is exactly the bug this
    guards against. recommendation_key is authoritative when present;
    the raw-score thresholds below only run for "copy"-tier wallets (to
    add Buy vs Strong Buy nuance) or as a last-resort fallback when no
    key was supplied at all.
    """
    if recommendation_key == "avoid":
        return "Avoid"
    if recommendation_key == "watch":
        return "Neutral"
    if recommendation_key == "copy":
        return "Strong Buy" if score >= 85 else "Buy"

    # Fallback only used if no recommendation_key was supplied at all.
    if score >= 85:
        return "Strong Buy"
    if score >= 72:
        return "Buy"
...
def _score_to_risk_level(score: int, recommendation_key: str = None) -> str:
    """See _score_to_recommendation_label - same reasoning applies here."""
    if recommendation_key == "avoid":
        return "High"
    if recommendation_key == "watch":
        return "Medium"
